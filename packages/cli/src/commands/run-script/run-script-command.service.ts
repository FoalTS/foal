...
export class RunScriptCommandService {
  constructor(
    private utilService: UtilService,
  ) {}

  async run(name: string, argv: string[]): Promise<void> {
    try {
      ...
      logger.initLogContext(() => this.execScript({ name }, argv, services, logger).catch(error => console.error(error)))
    } catch (error: any) {
  ...
  async execScript({ name }: { name: string }, argv: string[], services: ServiceManager, logger: Logger): Promise<void> {
    ...
    const args = this.utilService.getCommandLineArguments(argv);
