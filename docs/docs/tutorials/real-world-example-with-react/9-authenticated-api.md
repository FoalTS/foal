...
When sending a request to these endpoints, the `@UserRequired` hook will return a 401 error if the user has not previously logged in. But if they have, the controller method will be executed.

Go to [http://localhost:3001/swagger](http://localhost:3001/swagger) and check that the controller is working as expected.